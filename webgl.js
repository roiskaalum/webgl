...
function CreateCylinder(radius, height, colors) {
    ...
    // Draw side faces in reverse order
    for (let i = segments - 1; i >= 0; i--) {
        const next = (i - 1 + segments) % segments; // Wrap around backwards

        // Ensure consistent UV mapping for the first and last segments

        let uStart, uEnd;
        if (i === 0) {
            // Last face: start at 0, end at the value for the previous segment
            uStart = 0.0;
            uEnd = 1 - (next / segments);
        } else {
            uStart = 1 - (i / segments);
            uEnd = 1 - (next / segments);
        }
        ...
        AddTriangle(
            ...
            topVertices[i][0], topVertices[i][1], topVertices[i][2], ...colors[0], uCurrent, 1 - vCurrent, // Current vertex
            topVertices[next][0], topVertices[next][1], topVertices[next][2], ...colors[0], uNext, 1 - vNext,
